...
def add_options():
  ...
  flags.DEFINE_integer('batch', default = 16, help = 'number of trajectories collected parallely')
  ...
  for epoch in tqdm(range(FLAGS.epochs), desc = 'epoch'):
    ...
    for step in step_pbar:
      ...
      for traj in trajs:
        ...
        loss += -torch.mean(torch.min(surr1, surr2)) + 0.5 * criterion(pred_values, true_values)
      optimizer.zero_grad()
      loss.backward()
      optimizer.step()
